from test_TP09z import *

##################################
# Procédure d'exécution des tests
##################################

DICO  = {'01_rectangle': IntegrationRectangleTest,
           '02_trapeze': IntegrationTrapezeTest,
        '03_dichotomie': DichotomieTest,
          '04_triangle': TriangleTest,
             '05_gauss': GaussTest,
  '06_moyenne_variance': MoyenneVarianceTest,
               '07_imf': IMFTest,
           '08_etoiles': EtoilesTest,
            }
...
def detaille_note():
    ...
    for k in sorted(DICO.keys()):
        ...
        print("{:2d}/{:02d}".format(success,total),k)
        total_general += total
